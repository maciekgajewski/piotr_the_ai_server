...
usage() {
  echo "Usage: tools/ai-server-start.sh --services-config <path> --config <path>" >&2
}

die() {
  echo "ai-server-start.sh: $1" >&2
  usage
  exit 2
}
...
if docker compose version >/dev/null 2>&1; then
  docker compose --env-file "$SERVICES_CONFIG" -f docker-compose.yml -f docker-compose.ai-server.yml up -d ai-server
